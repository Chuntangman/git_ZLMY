      ...
      <div class="drawer-content">
        ...
        <div class="drawer-body">
          ...
          <div v-if="!isViewingDetails">
            <div v-if="loadingStates.relations" class="loading">
              加载中...
            </div>
            <div v-else-if="!selectedFeature" class="no-data">
              请选择一个要素以查看关联数据
            </div>
            <div v-else>
              <div v-if="errors.relations" class="error-message">
                {{ errors.relations }}
              </div>
              <div v-else-if="filteredRelations && filteredRelations.length > 0">
          ...
          <div v-else class="entity-details">
            ...
            <div class="tab-content">
              <div v-if="currentTab === 'basic'" class="tab-pane">
                <!-- Basic Info Content -->
                <div v-if="loadingStates.basic" class="loading">加载中...</div>
              ...
              <div v-else-if="currentTab === 'thin-section'" class="tab-pane">
                ...
                <div v-if="loadingStates['thin-section']" class="loading">加载中...</div>
              ...
              <div v-else-if="currentTab === 'xrf'" class="tab-pane">
                <div v-if="loadingStates.xrf" class="loading">加载中...</div>
...
export default {
  ...
  methods: {
    ...
    async findRelations(entityId) {
      ...
      this.loadingStates.relations = true;
      this.errors.relations = null;
      this.currentRelations = []; // 清空当前数据
      
      try {
        console.log('Finding relations for:', entityId);
        const relations = await getRelations(entityId);
        console.log('Found relations:', relations);
        
        // 确保返回的数据是数组
        this.currentRelations = Array.isArray(relations) ? relations : [];
        
      } catch (error) {
        console.error('Error loading relations:', error);
        this.errors.relations = error.message || '加载关联数据时发生错误';
        this.currentRelations = [];
      } finally {
        this.loadingStates.relations = false;
      }
    },
    ...
    async viewEntityDetails(entityId) {
      ...
      try {
        console.log('Fetching details for:', entityId);
    ...
    async fetchXRFDetails(id) {
      this.loadingStates.xrf = true;
      this.errors.xrf = null;
      try {
        console.log('Fetching XRF details for:', id);
        const data = await getXRFTestResults(id);
        console.log('Received XRF data:', data);
        this.xrfDetails = data;
      } catch (error) {
        console.error('Error fetching XRF details:', error);
        this.errors.xrf = error.message;
        this.xrfDetails = null;
      } finally {
        this.loadingStates.xrf = false;
      }
    },
    async fetchThinSectionDetails(id) {
      this.loadingStates['thin-section'] = true;
      this.errors['thin-section'] = null;
      try {
        console.log('Fetching thin section details for:', id);
        const details = await getThinSectionDetails(id);
        console.log('Received thin section details:', details);
        this.thinSectionDetails = details;
      } catch (error) {
        console.error('Error loading thin section details:', error);
        this.errors['thin-section'] = error.message;
        this.thinSectionDetails = null;
      } finally {
        this.loadingStates['thin-section'] = false;
      }
    }
  }
};
